...
import { BarChart3, ArrowLeft, TrendingUp, DollarSign, AlertCircle, CheckCircle, XCircle } from 'lucide-react';
import {
  ResponsiveContainer, ComposedChart, Bar, Line, XAxis, YAxis,
  Tooltip, Legend, ReferenceLine, CartesianGrid, Cell,
} from 'recharts';
...
function StatusIcon({ status }: { status: string }) {
  ...
  if (s === 'compliant') return <CheckCircle className="w-5 h-5 text-emerald-500 shrink-0" />;
  if (s === 'warning') return <AlertCircle className="w-5 h-5 text-amber-500 shrink-0" />;
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-6 space-y-5">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              {ffp.club_name} · Season {ffp.season_year}/{String(ffp.season_year + 1).slice(2)}
              {!ffp.revenue_configured && <span className="ml-2 text-amber-500">⚠ Revenue not set</span>}
            </p>
          </div>
        </div>
      ...
      <div className={cn(
        ...
        isCompliant ? 'bg-emerald-500/8 border-emerald-500/20' :
        ...
        <div className="flex-1 min-w-0">
          <p className={cn('font-bold text-sm',
            isCompliant ? 'text-emerald-600 dark:text-emerald-400' :
            isWarning   ? 'text-amber-600 dark:text-amber-400' :
                          'text-red-600 dark:text-red-400'
          )}>
            {status.badge || status.status || '—'}
          </p>
          {status.reason && <p className="text-xs text-muted-foreground mt-0.5">{status.reason}</p>}
          <div className="flex flex-wrap gap-3 mt-1.5 text-xs text-muted-foreground">
            <span>SCR: <strong>{formatPct(status.squad_cost_ratio)}</strong></span>
            <span>Break-even: <strong className={status.break_even_ok ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-500'}>{formatEur(status.break_even_result, true)}</strong></span>
          </div>
        </div>
      </div>
        ...
        <div className="flex flex-wrap items-center gap-2 p-3 bg-primary/8 border border-primary/20 rounded-xl text-xs animate-fade-up">
          ...
          <div className="ml-auto flex gap-3 text-muted-foreground">
            {ffp.simulation_extra_wages != null && <>
              <span className="text-red-500">+{formatEur(ffp.simulation_extra_wages, true)} wages</span>
              <span className="text-emerald-600 dark:text-emerald-400">-{formatEur(ffp.simulation_wage_relief ?? 0, true)} relief</span>
              <span>Net {formatEur(ffp.simulation_net_spend ?? 0, true)}</span>
            </>}
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 animate-fade-up">
        {[
          { label: 'Annual Revenue', value: formatEur(ffp.annual_revenue, true), sub: ffp.revenue_configured ? ffp.salary_data_source : 'Not configured', icon: <DollarSign className="w-4 h-4" /> },
          { label: 'Wage Bill', value: formatEur(ffp.current_wage_bill, true), sub: `${formatPct(ffp.current_wage_bill / (ffp.annual_revenue || 1))} of revenue`, icon: <TrendingUp className="w-4 h-4" /> },
          { label: 'Amortization', value: formatEur(ffp.current_amortization, true), sub: 'Transfer fee spread' },
          { label: 'Squad Cost Ratio', value: formatPct(ffp.current_squad_cost_ratio), sub: `Limit ${formatPct(ffp.squad_cost_ratio_limit)} · Warn ${formatPct(ffp.squad_cost_ratio_warning)}` },
        ].map(k => (
          <div key={k.label} className="bg-card border border-border rounded-2xl p-4">
            <div className="flex items-center justify-between mb-1">
              ...
              {k.icon && <span className="text-muted-foreground/40">{k.icon}</span>}
            </div>
      ...
      {projData.length > 0 && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 animate-fade-up">
          ...
          <div className="bg-card border border-border rounded-2xl p-4">
            <h3 className="font-semibold text-sm mb-4">Revenue vs Wage Bill (€M)</h3>
            <ResponsiveContainer width="100%" height={200}>
              <ComposedChart data={projData} margin={{ top: 5, right: 5, bottom: 0, left: -15 }}>
                ...
                <Tooltip content={<ChartTooltip />} cursor={{ fill: 'hsl(var(--secondary))', opacity: 0.5 }} />
                <Legend iconType="circle" iconSize={8} wrapperStyle={{ fontSize: 11, paddingTop: 8 }} />
                <Bar dataKey="Revenue" fill="hsl(var(--primary))" radius={[4, 4, 0, 0]} maxBarSize={40} />
                <Bar dataKey="Wage Bill" fill="hsl(var(--destructive))" opacity={0.8} radius={[4, 4, 0, 0]} maxBarSize={40} />
              </ComposedChart>
            </ResponsiveContainer>
          </div>
          ...
          <div className="bg-card border border-border rounded-2xl p-4">
            ...
            <ResponsiveContainer width="100%" height={200}>
              <ComposedChart data={projData} margin={{ top: 5, right: 10, bottom: 0, left: -15 }}>
                <CartesianGrid vertical={false} stroke="hsl(var(--border))" strokeOpacity={0.5} />
                <XAxis dataKey="year" tick={{ fontSize: 10, fill: 'hsl(var(--muted-foreground))' }} axisLine={false} tickLine={false} />
                <YAxis tick={{ fontSize: 10, fill: 'hsl(var(--muted-foreground))' }} axisLine={false} tickLine={false} domain={[0, 85]} />
                <Tooltip content={<ChartTooltip />} cursor={{ stroke: 'hsl(var(--border))' }} />
                <ReferenceLine y={70} stroke="#ef4444" strokeDasharray="4 3" strokeWidth={1.5}
                  label={{ value: '70% limit', position: 'insideTopRight', fontSize: 9, fill: '#ef4444' }} />
                <ReferenceLine y={65} stroke="#f59e0b" strokeDasharray="4 3" strokeWidth={1.5}
                  label={{ value: '65% warn', position: 'insideTopRight', fontSize: 9, fill: '#f59e0b' }} />
                <Line type="monotone" dataKey="SCR %" stroke="hsl(var(--primary))" strokeWidth={2.5}
                  dot={(props: any) => {
                    const { cx, cy, payload } = props;
                    return <circle key={payload.year} cx={cx} cy={cy} r={5} fill={statusColor(payload.status)} stroke="hsl(var(--card))" strokeWidth={2} />;
                  }}
                />
              </ComposedChart>
            </ResponsiveContainer>
      ...
      {ffp.projections.length > 0 && (
        <div className="bg-card border border-border rounded-2xl overflow-hidden animate-fade-up">
          <div className="px-4 py-3 border-b border-border flex items-center justify-between">
            ...
            <p className="text-xs text-muted-foreground">{ffp.projections.length} years</p>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-border bg-secondary/30">
                  {['Year', 'Revenue', 'Wages', 'Amort.', 'Squad Cost', 'SCR', 'Op. Result', 'FFP'].map(h => (
              ...
              <tbody>
                {ffp.projections.map(p => (
                  <tr key={p.year} className="border-b border-border/40 last:border-0 hover:bg-secondary/20 transition-colors">
                    <td className="py-2.5 px-3 font-semibold">{p.year}/{String(p.year + 1).slice(2)}</td>
                    ...
                    <td className="py-2.5 px-3">
                      <span className={cn('font-semibold',
                        p.squad_cost_ratio >= ffp.squad_cost_ratio_limit ? 'text-red-500' :
                        p.squad_cost_ratio >= ffp.squad_cost_ratio_warning ? 'text-amber-500' :
                        'text-emerald-600 dark:text-emerald-400'
                      )}>{formatPct(p.squad_cost_ratio)}</span>
                    </td>
                    <td className={cn('py-2.5 px-3 font-medium',
                      p.operating_result >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-500'
                    )}>{formatEur(p.operating_result, true)}</td>
