...
export default function ClubPage() {
  ...
  if (error || !club)
    return (
      <ErrorMessage message={(error as Error)?.message ?? "Club not found"} />
    );
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8 space-y-6">
      ...
      {/* Last sync */}
      <div
        className="flex items-center gap-1.5 text-xs text-muted-foreground animate-fade-up"
...
function RevenueModal({
  ...
  const {
    ...
    defaultValues: { revenue: currentRevenue / 1_000_000 },
    mode: "onSubmit",
  });
