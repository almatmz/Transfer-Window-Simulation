...
/** Two-step picker: search a club → browse its squad → select a player */
export function PlayerPicker({
  onSelect,
  placeholder = "Search a club to pick player…",
}: Props) {
  ...
  const pickClub = async (c: any) => {
    setClub(c);
    setStep("player");
    setLoading(true);
    setOpen(true);
    try {
      const data = await clubsApi.squad(c.api_football_id, CURRENT_SEASON);
  ...
  const inp =
    "w-full h-9 pl-9 pr-3 rounded-xl border border-input bg-background text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary placeholder:text-muted-foreground";
          ...
          {loading ? (
            ...
            filtered.slice(0, 15).map((p) => (
              ...
              >
                ...
                {(p.estimated_annual_salary ?? p.annual_salary) && (
