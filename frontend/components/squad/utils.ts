...
export function normalizePosition(pos: string | null | undefined): string {
  ...
  if (
    ...
    return "Defender";
  if (
    ["CM", "CDM", "CAM", "DM", "AM", "MF", "MIDFIELDER", "LM", "RM"].includes(p)
  )
    return "Midfielder";
  if (
    [
...
export function deduplicatePlayers(arr: any[]): any[] {
  ...
  return arr.filter((p) => {
    ...
    if (p.api_football_id > 0) {
      if (apiIds.has(p.api_football_id)) return false;
      apiIds.add(p.api_football_id);
    }
    return true;
  });
}
...
export function extractSquad(data: any): { players: any[]; expired: any[] } {
  ...
  for (const key of PLAYER_KEYS) {
    if (Array.isArray(data[key]) && data[key].length > 0) {
      const expired: any[] = EXPIRED_KEYS.reduce(
        (a: any[], k: string) => (Array.isArray(data[k]) ? data[k] : a),
        [] as any[],
      );
  ...
  for (const key of Object.keys(data)) {
    ...
    if (EXPIRED_KEYS.includes(key)) {
      expired = data[key];
      continue;
    }
    if (data[key].length > bestLen) {
      bestLen = data[key].length;
      best = key;
    }
  }
  return best
    ? { players: normalize(data[best]), expired: normalize(expired) }
    : { players: [], expired: [] };
}
