  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      await squadOverridesApi.create(clubId, {
        ...
        notes: [
          data.notes,
          data.loan_status === "loaned_out"
            ? `Loaned out to: ${data.loaned_out_to}`
            : "",
        ]
          .filter(Boolean)
          .join(" | "),
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4" noValidate>
        <div>
          <p className="text-xs text-muted-foreground mb-1.5">
            Search any club to auto-fill player details
          </p>
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              ...
              <label
                key={opt.value}
                className={`flex flex-col gap-0.5 p-3 rounded-xl border cursor-pointer transition-all ${loanStatus === opt.value ? "border-primary bg-primary/10 text-primary" : "border-border hover:border-primary/40"}`}
              >
                <input
                  type="radio"
          ...
          {loanStatus === "loaned_out" && (
            <div className="p-3 bg-orange-500/5 border border-orange-500/20 rounded-xl">
              <p className="text-xs text-orange-600 dark:text-orange-400 font-medium mb-2">
                Player is registered here but plays elsewhere on loan.
              </p>
              <input
                {...register("loaned_out_to")}
                placeholder="Loaned out to (e.g. Olympique Lyon)"
                className={inp}
              />
            </div>
          )}
          {loanStatus === "loan_in" && (
            <div className="p-3 bg-blue-500/5 border border-blue-500/20 rounded-xl">
              <p className="text-xs text-blue-600 dark:text-blue-400 font-medium mb-2">
                Player is owned elsewhere and plays here on loan.
              </p>
              <input
                {...register("loan_from_club")}
                placeholder="On loan from (e.g. FC Barcelona)"
                className={inp}
              />
            </div>
          )}
