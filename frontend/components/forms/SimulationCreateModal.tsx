...
export function SimulationCreateModal({ clubId, clubName, onClose, onCreated }: Props) {
  ...
  return (
    <Modal open onClose={onClose} title={`New Simulation — ${clubName}`}>
      <form onSubmit={handleSubmit(d => mut.mutate(d))} className="space-y-4">
        <input type="hidden" {...register('club_api_football_id', { valueAsNumber: true })} />
        <Input
        ...
        <div className="flex gap-2 justify-end">
          ...
          <Button type="submit" loading={mut.isPending}>Create Simulation</Button>
        </div>
      </form>
    </Modal>
  );
