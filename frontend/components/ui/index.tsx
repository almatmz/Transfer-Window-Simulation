...
export function Button({ className, variant = 'primary', size = 'md', loading, disabled, children, icon, ...props }: ButtonProps) {
  return (
    <button
      ...
      className={cn(
        ...
        variant === 'primary' && 'bg-primary text-primary-foreground hover:bg-primary/90 shadow-sm active:scale-[0.98]',
        ...
        variant === 'danger' && 'bg-destructive/10 text-destructive hover:bg-destructive hover:text-white border border-destructive/20',
        className
      )}
      {...props}
    >
...
interface InputProps extends React.InputHTMLAttributes<HTMLInputElement> {
  label?: string; error?: string; helperText?: string; leftIcon?: React.ReactNode;
}
  ...
  ({ className, label, error, helperText, id, leftIcon, ...props }, ref) => {
    ...
    return (
      <div className="space-y-1.5">
        ...
        {error && <p className="text-xs text-destructive flex items-center gap-1"><AlertCircle className="w-3 h-3 shrink-0" />{error}</p>}
...
export const Select = React.forwardRef<HTMLSelectElement, SelectProps>(
  ({ label, error, options, className, id, ...props }, ref) => {
    ...
    return (
      <div className="space-y-1.5">
        {label && <label htmlFor={selectId} className="block text-sm font-medium">{label}</label>}
        <select
          id={selectId} ref={ref}
          className={cn(
...
// ── Card ──────────────────────────────────────────────────────
interface CardProps extends React.HTMLAttributes<HTMLDivElement> { hover?: boolean; padding?: boolean; }
...
export function Badge({ className, variant = 'default', children, ...props }: BadgeProps) {
  return (
    <span className={cn(
      ...
      variant === 'success' && 'bg-emerald-500/10 text-emerald-600 border-emerald-500/20 dark:text-emerald-400',
      variant === 'warning' && 'bg-amber-500/10 text-amber-600 border-amber-500/20 dark:text-amber-400',
      variant === 'danger'  && 'bg-red-500/10 text-red-600 border-red-500/20 dark:text-red-400',
      variant === 'info'    && 'bg-primary/10 text-primary border-primary/20',
      variant === 'violet'  && 'bg-violet-500/10 text-violet-600 border-violet-500/20 dark:text-violet-400',
      className
    )} {...props}>
      {children}
    </span>
...
export function KpiCard({ label, value, sub, icon, highlight }: {
  label: string; value: string; sub?: string; icon?: React.ReactNode; highlight?: string;
}) {
  return (
    <Card className={cn('flex flex-col gap-1', highlight && `border-l-4 border-l-[${highlight}]`)}>
        ...
        >
          ...
          {t.count !== undefined && (
            <span className={cn(
              ...
              active === t.id ? 'bg-primary/10 text-primary' : 'bg-muted-foreground/20'
            )}>{t.count}</span>
          )}
        </button>
      ))}
...
interface PlayerOption { id: number; name: string; position?: string; photo_url?: string; }
export function PlayerSearchDropdown({
  players, onSelect, placeholder = 'Search players...',
}: {
  ...
  const filtered = q.length > 0
    ? players.filter(p => p.name.toLowerCase().includes(q.toLowerCase())).slice(0, 10)
    : players.slice(0, 8);
      ...
      {open && filtered.length > 0 && (
        <div className="absolute top-full mt-1 left-0 right-0 bg-popover border border-border rounded-xl shadow-xl z-50 overflow-hidden animate-scale-in">
          {filtered.map(p => (
            ...
            >
              {p.photo_url ? (
                <img src={p.photo_url} alt={p.name} className="w-7 h-7 rounded-full object-cover bg-muted" />
              ) : (
...
export function ClubSearchInput({
  label, value, onChange, placeholder = 'Search clubs...',
}: {
  label?: string; value: string; onChange: (clubId: number, clubName: string) => void; placeholder?: string;
}) {
        ...
        <div className="absolute z-50 mt-1 bg-popover border border-border rounded-xl shadow-xl overflow-hidden animate-scale-in w-80">
          {results.slice(0, 8).map((c: any) => (
            <button
              ...
              type="button"
              className="w-full flex items-center gap-3 px-3 py-2.5 text-sm hover:bg-secondary transition-colors text-left"
              onClick={() => { onChange(c.api_football_id, c.name); setQ(c.name); setOpen(false); }}
            >
              {c.logo_url && <img src={c.logo_url} alt={c.name} className="w-6 h-6 object-contain" />}
              <div>
...
export function FFPBadge({ status }: { status: string }) {
  ...
  return (
    <Badge variant={s === 'compliant' ? 'success' : s === 'warning' ? 'warning' : s === 'breach' ? 'danger' : 'default'}>
      {status || '—'}
    </Badge>
  );
}
...
export function PositionBadge({ position }: { position?: string }) {
  ...
  const color = p.startsWith('G') ? 'bg-amber-500/10 text-amber-600 border-amber-500/20'
    : p.startsWith('D') ? 'bg-blue-500/10 text-blue-600 border-blue-500/20'
    : p.startsWith('M') ? 'bg-emerald-500/10 text-emerald-600 border-emerald-500/20'
