...
@router.get("", response_model=list[SimulationResponse],
    summary="List all simulations for the logged-in user")
async def list_simulations(user: User = Depends(get_current_user)):
    sims = await transfer_service.list_user_simulations(str(user.id))
    results = []
    for sim in sims:
        transfers = await transfer_service.get_simulation_transfers(str(sim.id))
        results.append(_to_response(sim, transfers))
    return results


@router.post("", response_model=SimulationResponse, summary="Create new simulation")
async def create_simulation(body: CreateSimulationRequest, user: User = Depends(get_current_user)):
    sim = await transfer_service.create_simulation(
        user_id=str(user.id), club_api_football_id=body.club_api_football_id,
        name=body.name, season_year=body.season_year, window_type=body.window_type.value,
    )
    transfers = await transfer_service.get_simulation_transfers(str(sim.id))
    return _to_response(sim, transfers)


@router.get("/{sim_id}", response_model=SimulationResponse, summary="Get simulation with all transfers")
...
@router.delete("/{sim_id}", status_code=204, summary="Delete simulation and all its transfers")
...
@router.post("/{sim_id}/transfers", response_model=SimulationTransferResponse, summary="Add transfer to simulation")
async def add_transfer(sim_id: str, body: AddTransferRequest, user: User = Depends(get_current_user)):
    entry = await transfer_service.add_transfer(
        sim_id=sim_id, user_id=str(user.id),
        transfer_type=body.type.value, player_name=body.player_name,
        position=body.position, age=body.age, nationality=body.nationality,
        transfer_fee=body.transfer_fee, annual_salary=body.annual_salary,
        contract_length_years=body.contract_length_years,
        loan_fee=body.loan_fee, loan_fee_received=body.loan_fee_received,
...
@router.delete("/{sim_id}/transfers/{transfer_id}", status_code=204, summary="Remove a transfer from simulation")
...
def _transfer_to_response(t) -> SimulationTransferResponse:
    return SimulationTransferResponse(
        id=str(t.id), simulation_id=t.simulation_id, type=t.type.value,
        player_name=t.player_name, position=t.position, age=t.age,
        ...
        option_to_buy_fee=t.option_to_buy_fee, created_at=t.created_at,
    )


def _to_response(sim, transfers) -> SimulationResponse:
    return SimulationResponse(
        id=str(sim.id), club_name=sim.club_name, club_api_football_id=sim.club_api_football_id,
        name=sim.name, season_year=sim.season_year, window_type=sim.window_type.value,
        transfers=[_transfer_to_response(t) for t in transfers],
        is_public=sim.is_public, created_at=sim.created_at, updated_at=sim.updated_at,
    )